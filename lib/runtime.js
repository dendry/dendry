...
(function() {
  ...
  // Objects with this interface are passed to an interaction to have it
  // display content.
  ...
  // An interaction is given a runtime interface, the game and the
  // current game state (can be omitted). Interaction from the user
  // is given to the object to move the story on.
  var Interaction = function(runtime, game, state) {
  ...
  Interaction.prototype.displayCurrentScene = function() {
    if (this.isGameOver()) {
  ...
  Interaction.prototype.displayGameOver = function() {
    this.runtime.displayContent("Game Over");
    return this;
  };
  Interaction.prototype.displayScene = function(scene, options) {
    assert.ok(scene);
    assert.ok(options);
  ...
  Interaction.prototype.choose = function(optionIndex) {
    var scene = this.getCurrentScene();
    var options = this.getSceneOptions(scene);
    assert.ok(scene);
  ...
  Interaction.prototype.goToScene = function(id) {
    // TODO: Error checking on id.
    this.state.sceneId = id;
  };
  Interaction.prototype.getRootSceneId = function() {
    return this.game.firstScene || 'root';
  };
  Interaction.prototype.beginGame = function() {
    this.state = {};
  ...
  Interaction.prototype.gameOver = function() {
    this.state.sceneId = null;
    return this;
  };
  Interaction.prototype.isGameOver = function() {
    return this.getCurrentScene() === null;
  };
  Interaction.prototype.getCurrentScene = function() {
    var scene = this.game.scenes[this.state.sceneId];
    if (scene === undefined) return null;
    else return scene;
  };
  Interaction.prototype.getCurrentOptions = function() {
    var scene  = this.getCurrentScene();
    assert.ok(scene !== null);
    return this.getSceneOptions(scene);
  };
  Interaction.prototype.getSceneOptions = function(scene) {
    assert.ok(scene);
