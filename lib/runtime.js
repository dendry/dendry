...
(function() {
  ...
  GameState.prototype.__getChoiceData = function(idToTitleMap) {
    var result = [];
  ...
  GameState.prototype.__filterViewable = function(idToTitleMap) {
    var result = {};
    for (var id in idToTitleMap) {
  ...
  GameState.prototype.__getChoiceIdsFromOptions = function(options) {
    var that = this;
  ...
  GameState.prototype.__filterByPriority = function(choices,
                                                    minChoices, maxChoices) {
    // Early out if all choices are allowed.
    if (minChoices === null && maxChoices === null) return choices;
    ...
    if (maxChoices === null || maxChoices >= totalChoices) {
      ...
      committed.push.apply(committed, candidates);
    } else if (maxChoices >= committedChoices) {
      // The committed choices are already too many for the maximum
      // (usually happens when maxChoices < minChoices), so we can't
      // commit any candidates.
    } else {
      // Take a subset of the candidates, using their relative frequency.
      each(candidates, function(choice) {
        choice.selectionPriority = this.random() / choice.frequency;
      });
