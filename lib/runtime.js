...
(function() {
  ...
  GameState.prototype.displaySceneContent = function() {
    ...
    if (scene.onDisplay) {
      each(scene.onDisplay, function(fn) {
  ...
  GameState.prototype.goToScene = function(id, dontTransition) {
    ...
    if (!dontTransition && !!this.state.sceneId) {
      ...
      if (scene.onDeparture) {
        each(scene.onDeparture, function(fn) {
          fn(that, that.state, that.state.qualities);
        });
      }
    }
    ...
    if (!dontTransition && scene.onArrival) {
      each(scene.onArrival, function(fn) {
        fn(that, that.state, that.state.qualities);
      });
    }
  ...
  GameState.prototype._compileChoices = function(scene) {
    ...
    if (options !== undefined && options.options !== undefined) {
      ...
      for (id in choices) {
        ...
        var viewIf = this.game.scenes[id].viewIf;
        if (viewIf !== undefined) {
          var canView = viewIf(this, this.state, this.state.qualities);
          if (!canView) continue;
        }
