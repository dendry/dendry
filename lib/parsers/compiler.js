...
(function() {
  ...
  var getFullyQualifiedId = function(parentId, id, scenes) {
    ...
    throw new Error("Couldn't find an id matching '"+id+"' in '"+parentId+"'.");
  };
  ...
  Game.prototype._addSceneToScenes = function(scene, parentId, callback) {
    ...
    if (this.scenes[id] !== undefined) {
      return callback(new Error("Duplicate scenes with id '"+id+"' found."));
    }
  ...
  Game.prototype._fullyQualifyIdsInOptions = function(scene, callback) {
    ...
    async.each(scene.options, function(option, done) {
      ...
      that._fullyQualifyIdsInFunctionByName(scene, option, 'viewIf', done);
    }, callback);
  };
  ...
  Game.prototype._fullyQualifyIdsInFunction = function(scene, fn, callback) {
    ...
    try {
      source = original.replace(idInLogicRe, function(_, sourceId) {
        return '@'+getFullyQualifiedId(scene.id, sourceId, that.scenes);
      });
    } catch(err) {
      return callback(err);
    }
  ...
  Game.prototype._fullyQualifyIdsInFunctionByName =
    function(scene, object, name, callback) {
      var fn = object[name];
      if (fn === undefined) return callback(null);
      ...
      if (_.isArray(fn)) {
        ...
        async.each(indices, function(i, done) {
          var thisFn = fns[i];
          that._fullyQualifyIdsInFunction(object, thisFn, function(err, fn) {
            if (err) return done(err);
            if (fns[i] !== fn) fns[i] = fn;
            return done(null);
          });
        }, callback);
      } else {
        this._fullyQualifyIdsInFunction(scene, fn, function(err, fn) {
          if (err) return callback(err);
          if (object[name] !== fn) object[name] = fn;
          return callback(null);
        });
      }
    };

  var sceneFunctions = ['viewIf', 'onArrival', 'onDeparture', 'onDisplay'];
  ...
  Game.prototype._fullyQualifyIdsInScene = function(scene, callback) {
    ...
    this._fullyQualifyIdsInGoto(scene, function(err) {
      ...
      that._fullyQualifyIdsInOptions(scene, function(err) {
        ...
        async.each(sceneFunctions, function(name, done) {
          that._fullyQualifyIdsInFunctionByName(scene, scene, name, done);
        }, callback);
      });
    });
  };
  ...
  var compile = function(infoDry, listOfScenes, listOfQualities, callback) {
    ...
    result._addScenes(listOfScenes, function(err) {
      if (err) return callback(err);

      // Compile the initial list of quality values.
      result._addQualities(listOfQualities, function(err) {
        // Add qualities cannot currently return an error.
        assert(!err);

        // Make sure all cross references are wired up.
        result._link(function(err) {
          if (err) return callback(err);
