...
(function() {
  ...
  BoundaryFinder.prototype.add = function(featureName, type, start, end) {
    ...
    this.result.push({feature:featureName, type:type, start:start, end:end});
  };
  ...
  var _extractInsert = function(range, output) {
    ...
    if (content.length === 1) {
      ...
      if (content[0].type === 'magic') {
        ...
        if (content[0].type !== undefined) illegalFormat();
      ...
      // Magic and qdisplay.
      if (content.length !== 2) illegalFormat();
      if (content[0].type !== 'magic') illegalFormat();
      if (content[1].type !== undefined) illegalFormat();
      if (content[1].substr(0, 1) !== ':') illegalFormat();

      source = content[0].content.join('; ');
      qdisplay = content[1].substring(1).trim();
      expression = {type:'insert', language:'magic',
                    source: source, qdisplay: qdisplay};
    }
  ...
  var compile = function(text, findParagraphs, callback) {
    ...
    try {
      ...
      var boundaries = _createFeatureBoundaries(text, features, findParagraphs);
