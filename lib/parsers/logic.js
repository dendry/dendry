...
(function() {
  ...
  var _fn = function(sequence, _) {
    if (sequence.length == 4) {
      return _sequence(['state.functions.', sequence[0].text, '('],
                       sequence[2], [')']);
    } else {
      return _sequence(['state.functions.', sequence[0].text, '()']);
    }
  ...
  var compile = function(source, root, callback) {
    tokenizer.run(source, function(err, tokens) {
      if (err) return callback(err);
      parser.run(tokens, root, function(err, parse) {
        if (err) return callback(err);
        var jsSource = parse.object.join('');
        /*jshint -W054 */
        var fn = new Function('engine', 'state', 'Q', jsSource);
        /*jshint +W054 */
        fn.source = source;
        fn.jsSource = jsSource;
