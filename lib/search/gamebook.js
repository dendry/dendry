...
(function() {
  ...
  var build = function(game, limit) {
    ...
    if (enumeratedStates.hasReachedSearchLimit) {
      throw new Error(
        'Reached search limit of ' + enumeratedStates.numStatesFound +
        ' sections.'
        );
    }
    ...
    var gamebook = [];
    var numIndices = outputIndexToStateIndex.length;
    for (var outputIndex = 0; outputIndex < numIndices; ++outputIndex) {
      ...
      for (var j = 0; j < ui.choices.length; ++j) {
        var choiceJ = ui.choices[j];
        if (choiceJ.canChoose) {
