...
(function() {
  ...
  StateEnumerator.prototype.enumerate = function() {
    ...
    var seeState = function(parentIndex, optionIndex) {
      ...
      if (that.storeStates && parentIndex !== undefined) {
        searchStates[parentIndex].validChoiceDestinations.push(stateIndex);
    ...
    var processNext = function() {
      ...
      var option = choiceIndexStack.pop();

      // Get the resulting state from making this choice.
      de.setState(beforeState);
      var currentChoices = de.getCurrentChoices();
      var canChoose = !!currentChoices[option].canChoose;
      if (canChoose) {
        de.choose(option);
      }
      ...
      ++option;
      if (option >= currentChoices.length) {
    ...
    if (this.storeStates) {
      result.searchStates = searchStates;
    }
    return result;
  };
  ...
  var _makeEnumerator = function(game, limit, goalSceneId, rndSeeds) {
    ...
    if (goalSceneId !== undefined) {
      enumerator.goalSceneId = goalSceneId;
    }
    if (rndSeeds !== undefined) {
      enumerator.rndSeeds = rndSeeds;
    }
    return enumerator;
  };
