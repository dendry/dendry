    ...
    var getFullyQualifiedId = function(parentId, id) {
      ...
      if (result.scenes[id] !== undefined) {
        ...
        if (id.substr(0,1) === '.') {
          ...
          var up = 1;
          while (id.substr(up, 1) === '.') ++up;
          if (parentComponents.length < up-1) {
            callback(
              new Error("Can't go up "+(up-1)+" levels from '"+parentId+"'.")
            );
            return null;
          }
          // Go up the right number of levels.
          var restOfId = id.substr(up);
          var baseId = parentId;
          if (up > 1) {
            baseId = parentComponents.slice(0, -up+1).join(".");
          }
          // Build the full id and test it.
          var idBits = [];
          if (baseId.length > 0) idBits.push(baseId);
          if (restOfId.length > 0) idBits.push(restOfId);
          var testId = idBits.join('.');
          if (result.scenes[testId] !== undefined) {
            newId = testId;
          }
        } else {
          // Work from global to local finding this id.
