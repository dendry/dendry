...
(function() {
  ...
  Parser.prototype.run = function(tokens, root, callback) {
    ...
    for (i = 0; i < tokens.length+1; i += 1) {
      ...
      if (currentStates === undefined || currentStates.length <= 0) break;
    ...
    if (parses.length === 1) {
      ...
      return callback(new Error("No valid way to parse this content."));
    } else {
      return callback(new Error("Content is ambiguous, "+parses.length+
                                " ways to parse it."));
    }
  };
