...
(function() {
    ...
    while (pos < len) {
      ...
      if (matchRule === null) {
        if (!this.generateNullToken) {
          return callback(new Error("Unrecognized content at "+pos+"."));
        } else {
  ...
  Parser.prototype.run = function(tokens, root, callback) {
    root = root || 'root';
    var i;
    ...
    if (this.states[0].length === 0) {
      return callback(new Error("I couldn't parse this. [Error G101]"));
    }
    ...
    if (parses.length > 0) {
      ...
      var that = this;
      return callback(null, _.map(parses, function(parse) {
        return that._buildParseTree(tokens, parse);
      }));
    }

    // We had a problem.
    return callback(new Error("I couldn't parse this. [Error P101]"));
  };
      ...
      for (var j = 0; j < rule.children.length; j += 1) {
        ...
        if (child.type === Parser.NON_TERMINAL) {
          ...
          if (!ruleNames[child.name]) {
            throw new Error("Non-terminal "+child.name+
  ...
  Parser.prototype._buildParseTree = function(tokens, state) {
    ...
    if (stack.length === 1) {
      ...
      throw new Error("I could't parse this. [Error P102]");
    }
  };
