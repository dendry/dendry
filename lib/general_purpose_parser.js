...
(function() {
    ...
    while (pos < len) {
      ...
      if (matchRule === null) {
        if (!generateNullToken) {
          throw new Error("Unrecognized content at "+pos);
        } else {
      ...
      j += 1;
      if (j > 100) {
        throw new Error("Runaway tokenizing.");
      }
    }
  ...
  Parser.prototype.run = function(tokens, root) {
    ...
    if (this.states[0].length === 0) {
      throw new Error("I couldn't parse this. [Error G101]");
    }
    ...
    if (parses.length > 0) {
      ...
      var that = this;
      return _.map(parses, function(parse) {
        that._buildParseTree(tokens, parse);
      });
    }

    // We had a problem.
    throw new Error("I couldn't parse this. [Error P101]");
  };
  ...
  Parser.prototype._checkGrammar = function() {
    ...
    // being fired.
    _.each(ruleNames, function(name) {
