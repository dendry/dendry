...
(function() {
  ...
  UserInterface.prototype.displayContent = function(content, predicates) {};
  UserInterface.prototype.displayChoices = function(choices) {};
  UserInterface.prototype.displayGameOver = function() {
    this.displayContent([{type:'paragraph', content:["Game Over"]}]);
  };
  ...
  DendryEngine.prototype.displaySceneContent = function() {
    ...
    if (scene.content) {
      var predicates = [];
      if (scene.content.predicates !== undefined) {
        for (var i = 0; i < scene.content.predicates.length; ++i) {
          predicates.push(this._runPredicate(scene.content.predicates[i]));
        }
      }
      var chunks = scene.content.chunks;
      if (chunks === undefined) {
        chunks = [{type:'paragraph', content:[scene.content]}];
      }
      this.ui.displayContent(chunks, predicates);
    }
