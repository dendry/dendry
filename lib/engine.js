...
(function() {
  ...
  DendryEngine.prototype.__getChoiceData = function(idToInfoMap) {
    ...
    for (var id in idToInfoMap) {
      ...
      optionInfo.subtitle = optionInfo.subtitle || optionScene.subtitle;
  ...
  DendryEngine.prototype._compileChoices = function(scene) {
    ...
    if (options !== undefined) {
      ...
      // Map to displayable content and check whether it can be chosen.
      for (var i = 0; i < validChoiceData.length; ++i) {
        var choice = validChoiceData[i];
        var canChoose = true;
        if (choice.chooseIf) {
          canChoose = this._runPredicate(choice.chooseIf, true);
        }
        if (canChoose) {
          // Only check with the scene if we've not already been vetoed.
          var choiceScene = this.game.scenes[choice.id];
          if (choiceScene.chooseIf) {
        ...
        var finalChoice = {
          id:choice.id,
          canChoose:canChoose,
          title:this._makeDisplayContent(choice.title, false)
        };
        if (choice.subtitle) {
          finalChoice.subtitle =
            this._makeDisplayContent(choice.subtitle, false);
        }
        choiceOutput.push(finalChoice);
        if (canChoose) ++choosableOptions;
      }
    }

    if (choosableOptions === 0) {
