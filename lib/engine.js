  ...
  DendryEngine.prototype._compileChoices = function(scene) {
    ...
    if (options !== undefined) {
      ...
      for (var i = 0; i < validChoiceData.length; ++i) {
        ...
        choiceOutput.push(
          {id:choice.id, canChoose:canChoose,
           title:this._makeDisplayContent(validChoiceData[i].title, false)}
          );
    ...
    if (choosableOptions === 0) {
      ...
      if (root !== this.state.sceneId) {
        ...
        if (!rootSceneChoose || this._runPredicate(rootSceneChoose, true)) {
          choiceOutput.push({id:root, title:'Scene Complete', canChoose:true});
          ++choosableOptions;
        }
      }
    }
