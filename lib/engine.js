...
(function() {
  ...
  DendryEngine.prototype.displaySceneContent = function() {
    ...
    if (scene.signal !== undefined) {
      this.ui.signal(scene.signal, {signal:scene.signal,
                                    type:"scene-display",
  ...
  DendryEngine.prototype.goToScene = function(id) {
    ...
    if (!!fromId) {
      ...
      if (from.signal !== undefined) {
        this.ui.signal(from.signal, {signal:from.signal,
                                     type:"scene-departure",
                                     id:this.state.sceneId,
                                     "to":id});
      }
    }
    ...
    if (scene.signal !== undefined) {
      var signal = {signal:scene.signal,
                    type:"scene-arrival",
                    id:id};
      if (!!fromId) signal.from = fromId;
      this.ui.signal(scene.signal, signal);
    }
      ...
      if (needsAccessors) {
        ...
        Q.__defineSetter__(id, function(value) {
          ...
          if (signal !== undefined && value !== was) {
            ...
            that.ui.signal(signal, signalObj);
          }
        });
      }
