...
(function() {
  ...
  DendryEngine.prototype.displaySceneContent = function() {
    ...
    if (scene.content) {
      var stateDeps = [];
      if (scene.content.stateDependencies !== undefined) {
        for (var i = 0; i < scene.content.stateDependencies.length; ++i) {
          stateDeps.push(
            this._runPredicate(scene.content.stateDependencies[i])
          );
        }
      }
