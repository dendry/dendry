...
(function() {
  ...
  UserInterface.prototype.displayContent = function(paragraphs, data) {};
  ...
  DendryEngine.prototype.displaySceneContent = function() {
    ...
    if (scene.content) {
      var stateDeps = [];
      var stateDepDefs = scene.content.stateDependencies;
      if (stateDepDefs !== undefined) {
        for (var i = 0; i < stateDepDefs.length; ++i) {
          var stateDepDef = stateDepDefs[i];
          var fn = stateDepDef.fn;
          switch(stateDepDef.type) {
          case 'insert':
            stateDeps.push(this._runExpression(fn));
            break;

          default:
            assert(stateDepDef.type === 'predicate');
            stateDeps.push(this._runPredicate(fn));
            break;
          }
        }
      }
      var paragraphs = scene.content.paragraphs;
      if (paragraphs === undefined) {
        paragraphs = [{type:'paragraph', content:[scene.content]}];
      }
      this.ui.displayContent(paragraphs, stateDeps);
    }
