...
(function() {
  ...
  var validatePredicate = function(property, callback) {
    ...
    if (magicStart > -1 || magicEnd > -1) {
      ...
      if (magicStart > 0 || magicEnd < val.length-2) {
        ...
        try {
          var fn = runtime.makeFunctionFromSource(magic);
      ...
      // Parse this as Logic (TODO).
      err = properr(property, "Logic is not currently supported, use Magic");
      err.property = property;
      return callback(err);
    }
  };
  ...
  var validateActions = function(property, callback) {
    ...
    for (i = 0; i < chunks.length; ++i) {
      ...
      if (chunk.type === 'magic') {
        try {
          fn = runtime.makeFunctionFromSource(chunk.source);
          actions.push(fn);
        } catch(evalErr) {
        ...
        err = properr(
          property,
          "Logic is not currently supported, use Magic in chunk "+(i+1)
        );
        err.property = property;
        return callback(err);
      }
