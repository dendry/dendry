...
(function() {
  ...
  // Accessor TODO allows us to refactor to expose property line numbers.
  ...
  var makeEnsureEqualTo = function(value) {
    ...
    return function(property, callback) {
      ...
      if (val === value) {
        ...
        return callback(properr(property, "Property must equal '"+value+"', '"+
                                val+"' found instead."));
      }
    };
  };
  ...
  var validateTagList = function(property, callback) {
    ...
    for (var i = 0; i < tags.length; ++i) {
      ...
      if (!match) {
        var err = properr(property, "Tag "+(i+1)+" is not valid.");
        err.property = property;
        return callback(err);
      } else {
        result.push(match[1]);
  ...
  var makeEnsureListItemsMatchSchemaById = function(schemae) {
    return function(property, callback) {
      ...
      var result = [];
      async.each(list, function(item, done) {
        var schema = schemae[item.id];
        if (schema === undefined) schema = schemae.$default;
        if (schema === undefined) {
          return done(properr(property, "Found an item with an unknown id '"+
                              item.id+"'."));
        }
        var ensureItem = makeEnsureObjectMatchesSchema(schema);
        ensureItem(item, function(err, resultItem) {
