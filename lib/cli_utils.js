...
(function() {
  ...
  var loadCompiledGame = function(path, callback) {
  ...
  var convertGameToJSON = function(game, callback) {
    var replacer = function(key, value) {
      if (_.isFunction(value)) {
        if (value.source === undefined) {
          throw new Error("Function "+key+" does not have source.");
        }
    ...
    try {
      var json = JSON.stringify(game, replacer, 2);
  ...
  // Alias this, because we want to define it in runtime (so runtime
  // has fewer dependencies), but it is more logically here with its
  // counterpart, convertGameToJSON.
  var convertJSONToGame = runtime.convertJSONToGame;
