...
(function() {
  ...
  CommandLineUserInterface.prototype.displayContent = function(content, width) {
    width = width || process.stdout.getWindowSize()[0] - 2;
    var text;
    if (content.chunks !== undefined) {
      text = this._contentToText(content.chunks, width);
    } else {
      text = wordwrap(width)(content)+'\n';
    }
  ...
  CommandLineUserInterface.prototype._contentToText = function(content, w) {
    var that = this;
    var text;
    var result = [];
    for (var i = 0; i < content.length; ++i) {
      ...
      if (element.type === undefined) {
        ...
        switch(element.type) {
        case 'heading':
          result.push(that._contentToText(element.content, w).bold);
          result.push((i === content.length - 1) ? "\n" : "\n\n");
          break;
        case 'paragraph':
          text = that._contentToText(element.content, w);
        ...
        case 'emphasis-1':
          result.push(' ');
          result.push(that._contentToText(element.content, w));
          result.push(' ');
          break;
        case 'emphasis-2':
          result.push(' ');
        ...
        case 'hidden':
          result.push(' ');
          result.push(that._contentToText(element.content, w).grey);
          result.push(' ');
          break;
/*
        case 'conditional':
          // TODO:
          result.push(' ');
          result.push(that._contentToText(element.content, w).red);
          result.push(' ');
          break;
*/
        case 'line-break':
