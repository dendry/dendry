...
(function() {
  ...
  CommandLineUserInterface.prototype.displayContent =
    function(paragraphs, data) {
      var width = this.defaultWidth || process.stdout.getWindowSize()[0] - 2;
      var text = this._paragraphsToText(paragraphs, data, width);
  ...
  CommandLineUserInterface.prototype._contentToText = function(content, data) {
    var that = this;
    var text;
    var result = [];
    for (var i = 0; i < content.length; ++i) {
      ...
      if (range.type === undefined) {
        ...
        case 'emphasis-1':
          result.push(' ');
          result.push(that._contentToText(range.content, data));
          result.push(' ');
          break;
        case 'emphasis-2':
          result.push(' ');
          result.push(that._contentToText(range.content, data).bold);
          result.push(' ');
          break;
        case 'hidden':
          result.push(' ');
          result.push(that._contentToText(range.content, data).grey);
          result.push(' ');
          break;
        case 'conditional':
  ...
  CommandLineUserInterface.prototype._paragraphsToText =
    function(paragraphs, data, width) {
      var that = this;
      var text;
      var result = [];
      for (var i = 0; i < paragraphs.length; ++i) {
        ...
        case 'heading':
          result.push(that._contentToText(paragraph.content, data).bold + '\n');
          break;
        case 'paragraph':
          text = that._contentToText(paragraph.content, data);
