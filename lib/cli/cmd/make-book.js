...
(function() {
  ...
  var buildGamebook = function(data, callback) {
    var result = gamebook.build(data.game);
  ...
  cmdMakeBook.createArgumentParser = function(subparsers) {
    var parser = subparsers.addParser(this.name, {
      ...
      description: "Builds an ebook version of a game, compiling it first "+
        "if it is out of date. The ebook generation uses a template which "+
        "can be a predefined template, or the path to a directory. Generates "+
        "a book in the HTML-based EPUB format, which can be converted into "+
        "Kindle format using Amazon's free kindlegen tool."
    });
