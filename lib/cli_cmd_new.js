...
(function() {
  ...
  var getTemplateDir = function(data, callback) {
    cliUtils.getTemplatePath(
      data.template, "new",
      function(err, sourceDir, name) {
        if (err) return callback(err);
        // Make sure it is a project.
        cliUtils.getProjectPath(sourceDir, function(err, sourceDir) {
          if (err) return callback(err);
  ...
  var transformAndCopy = function(data, callback) {
    cliUtils.copyTemplate(
      data.sourceDir, data.destDir, data, true,
      function(err) {
        if (err) return callback(err);
  ...
  var cmdNew = new cliUtils.Command("new");
