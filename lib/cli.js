...
(function() {
  ...
  cmdNew.run = function(args, callback) {
    ...
    var getSourceDir = function(data, callback) {
      ...
      if (!fs.existsSync(sourceDir)) {
        sourceDir = path.resolve(__dirname, "templates", "new", sourceDir);
        if (!fs.existsSync(sourceDir)) {
  ...
  cmdCompile.run = function(args, callback) {
    ...
    var ensureDirectory = function(data, callback) {
      ...
      var infoPath = path.join(sourceDir, "info.dry");
      if (!fs.existsSync(infoPath)) {
        return callback(new Error("No info file in project: "+sourceDir));
      }
      data.sourceDir = sourceDir;
      callback(null, data);
    };
    ...
    var compileGame = function(data, callback) {
      if (!data.needsCompilation) {
        ...
        if (data.command !== 'compile') {
          console.log(
            "Game file is out of date, recompiling.".red
          );
        }
      }
  ...
  // ----------------------------------------------------------------------
  // Build-HTML: Creates a playable HTML version of the game.
  // ----------------------------------------------------------------------

  var cmdBuildHTML = new Command("build-html");
  cmdBuildHTML.createArgumentParser = function(subparsers) {
    var parser = subparsers.addParser(this.name, {
      help: "Build a project into a playable HTML page."
    });
    parser.addArgument(['project'], {
      nargs: "?",
      help: "The project to compile (default: the current directory)."
  ...
  cmdBuildHTML.run = function(args, callback) {
    // Optionally compile, then build the HTML
    var getData = function(callback) {
      cmdCompile.run(args, callback);
    };
    ...
    var writeHTML = function(data, callback) {
      var outPath = path.join(data.sourceDir, "output.html");
