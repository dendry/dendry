  ...
  cmdNew.run = function(args, callback) {
    ...
    var doCmd = function(params) {
      ...
      if (!fs.existsSync(source_diry)) {
        ...
        if (!fs.existsSync(source_diry)) {
          var err = new Error("Cannot find template: "+template);
          return callback(err);
        } else {
          var info_path = path.join(source_diry, "_info.dry");
          if (!fs.existsSync(info_path)) {
            var err = new Error("Can't find info file in template: "+info_path);
            return callback(err);
          }
        }
      }
      ...
      if (fs.existsSync(dest_diry)) {
        var err = new Error("Destination directory already exists: "+dest_diry);
        return callback(err);
      }

      // Setup the handlebars context.
      params.id = id;
      params.diry = dest_diry;
