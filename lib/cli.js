...
(function() {
  ...
  cmdValidate.run = function(args, callback) {
    ...
    var ensureInfoFile = function(data, callback) {
      var infoPath = path.join(data.sourceDir, "info.dry");
      if (!fs.existsSync(infoPath)) {
  ...
  cmdRun.run = function(args, callback) {
    ...
    var checkCompiledGameUpToDate = function(data, callback) {
      ...
      if (fs.existsSync(data.compiledPath)) {
        // Check its date.
    ...
    async.waterfall(
      [getData, ensureDirectory, checkCompiledGameUpToDate, compileGame,
       saveGame, loadGame, runGame], callback);
  };
